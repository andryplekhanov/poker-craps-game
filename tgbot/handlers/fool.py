...
from tgbot.keyboards.inline_fool import fool_start_game, propose_more_cards, show_done_button
...
from tgbot.services.fool_service import create_deck, play_fool_round, hand_out_cards, pick_card, place_card_on_desk, \
    bot_try_cover, check_more_cards
...
async def player_give_more(call: CallbackQuery, callback_data: dict, state: FSMContext):
    await call.message.edit_reply_markup(reply_markup=None)
    card = callback_data.get('card')
    await place_card_on_desk(call.message, state, card, place_for='player')
...
def register_fool(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(player_give_more, for_fool_player_give_more.filter(), state="*")
